...
//controllerfor enhancing a resumes's professional summary
...
export const uploadResume = async (req, res) => {
    try {
        ...
        if(!resumeText || resumeText.trim().length === 0){
            return res.status(400). json( {message: 'Resume content is required for upload'} );
        }
        ...
        // store under `user` to match Resume schema
        const newResume = await Resume.create( { user: userId, title, ...parsedData} );

        // return the full resume object to match other endpoints
